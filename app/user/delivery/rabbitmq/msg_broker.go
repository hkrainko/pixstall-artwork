...
import (
	...
	"pixstall-artwork/domain/user/model"
	"time"
)
...
func NewUserMessageBroker(artworkUseCase artwork.UseCase, conn *amqp.Connection) UserMessageBroker {
	ch, err := conn.Channel()
	if err != nil {
...
func (u UserMessageBroker) consumeUserEventUpdated(ctx context.Context, body []byte) error {
	...
	return u.artworkUseCase.UpdateArtworkByUserUpdatedEvent(ctx, req)
}
